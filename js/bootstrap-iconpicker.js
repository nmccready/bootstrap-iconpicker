...
(function($) {
  ...
  Iconpicker.prototype.select = function(icon) {
    ...
    if (op.selected === -1) {
      op.selected = 0;
      icon = op.iconClassFix + op.icons[op.selected]; // eslint-disable-line no-param-reassign
    }
  ...
  Iconpicker.prototype.updateIconsCount = function() {
    ...
    if (op.footer === true) {
      var iconsCount = [
        '<tr>', '   <td colspan="' + op.cols + '" class="text-center">',
        '       <span class="icons-count"></span>',
        '   </td>',
        '</tr>'
      ];
      op.table.find('tfoot').empty().append(iconsCount.join(''));
    }
  };

  Iconpicker.prototype.updateLabels = function(page) {
    var op = this.options;
    var totalIcons = this.totalIcons();
    var totalPages = this.totalPages();
    op.table.find('.page-count').html(op.labelHeader.replace('{0}', (totalPages === 0)
      ? 0
      : page).replace('{1}', totalPages));
    var offset = (page - 1) * this.totalIconsPerPage();
    var total = page * this.totalIconsPerPage();
    op.table.find('.icons-count').html(op.labelFooter.replace('{0}', totalIcons
      ? offset + 1
      : 0).replace('{1}', (total < totalIcons)
      ? total
      : totalIcons).replace('{2}', totalIcons));
  ...
  Iconpicker.prototype.updatePagesCount = function() {
    ...
    if (op.header === true) {
      ...
      for (var i = 0; i < op.cols; i++) {
        ...
        if (i === 0 || i === op.cols - 1) {
          var arrow = [
            '<button class="btn btn-arrow ' + ((i === 0)
              ? 'btn-previous'
              : 'btn-next') + ' ' + op.arrowClass + '" value="' + ((i === 0)
              ? -1
              : 1) + '">',
            '<span class="' + ((i === 0)
              ? op.arrowPrevIconClass
              : op.arrowNextIconClass) + '"></span>',
            '</button>'
          ];
  ...
  $.fn.iconpicker = function(option, params) {
    return this.each(function() {
      ...
      if (typeof option === 'string') {
        ...
        var name = (typeof $this.attr('name') !== 'undefined')
          ? 'name="' + $this.attr('name') + '"'
          : '';

        if ($this.prop('tagName') === 'BUTTON') {
          $this.empty().append('<i></i>')
          ...
          $this.empty()
            .append('<input type="hidden" ' + name + '></input>')
            .append(op.table).addClass('iconpicker')
            .addClass(op.align);
          data.switchPage(op.icon);
          data.bindEvents();
        }
